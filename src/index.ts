...
class KyveCosmos extends KYVE {
  ...
  constructor() {
    ...
    const root = new Root();
    root.loadSync(path.join(__dirname, "schema.proto"), { keepCase: true });
  ...
  public async requestWorkerBatch(workerHeight: number): Promise<any[]> {
    ...
    const currentHeight = await provider.fetchCurrentHeight();
    const promises: any[] = [];
    ...
    for (let height = workerHeight; height < toHeight; height++) {
      promises.push(provider.fetchBlock(height));
